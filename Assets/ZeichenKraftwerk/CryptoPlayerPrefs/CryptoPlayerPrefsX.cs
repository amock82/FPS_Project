...
public class CryptoPlayerPrefsX
{
	...
	{
		if (CryptoPlayerPrefs.HasKey (key)) {
			var bytes = System.Convert.FromBase64String (CryptoPlayerPrefs.GetString (key));
			if (bytes.Length < 6) {
	...
	public static bool SetStringArray (String key, String[] stringArray)
	{
		...
		try		{
			CryptoPlayerPrefs.SetString (key, System.Convert.ToBase64String (bytes) + "|" + String.Join ("", stringArray));
		} catch {
			return false;
		}
		return true;
	...
	{
		if (CryptoPlayerPrefs.HasKey (key)) {
			var completeString = CryptoPlayerPrefs.GetString (key);
	...
	{
		try		{
			CryptoPlayerPrefs.SetString (key, System.Convert.ToBase64String (bytes));
		} catch {
			return false;
		}
		return true;
